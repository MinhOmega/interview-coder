...
if (process.platform === "darwin") {
  ...
  app.setAsDefaultProtocolClient("interview-coder", process.execPath, [path.resolve(process.argv[1] || "")]);
}
...
function hideMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    // Update shortcuts to only allow visibility toggle
    state.shortcutsHelper?.updateShortcutsForVisibility(false);
  }
}
...
function showMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    // Update shortcuts to allow all shortcuts
    state.shortcutsHelper?.updateShortcutsForVisibility(true);
  }
}
...
function toggleMainWindow(): void {
  ...
  // Update shortcuts based on new visibility
  state.shortcutsHelper?.updateShortcutsForVisibility(state.isWindowVisible);
}
...
async function initializeApp() {
  try {
    ...
    const appDataPath = path.join(app.getPath("appData"), "interview-coder-assistant");
    ...
    // Log current visibility and shortcut status
    console.log(`Initial window visibility: ${state.isWindowVisible ? "visible" : "hidden"}`);
    console.log(`Shortcuts initialized in ${state.isWindowVisible ? "full" : "toggle-only"} mode`);
...
// Set up IPC listeners for deleting the last screenshot
ipcMain.on("delete-last-screenshot", async () => {
  try {
    if (state.screenshotHelper) {
      const result = state.screenshotHelper.deleteLastScreenshot();
      if (result.success) {
        state.mainWindow?.webContents.send("last-screenshot-deleted", { success: true, path: result.path });
      } else {
        state.mainWindow?.webContents.send("last-screenshot-deleted", { success: false, error: result.error });
      }
    } else {
      state.mainWindow?.webContents.send("last-screenshot-deleted", {
        success: false,
        error: "Screenshot helper not initialized",
      });
    }
...
// Add the captureArea function
async function captureArea(): Promise<void> {
  if (state.mainWindow) {
    state.mainWindow.webContents.send("start-area-capture");
  }
}
