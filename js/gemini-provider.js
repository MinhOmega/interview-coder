  ...
  try {
    ...
    try {
      ...
      if (response.data && response.data.models) {
        // API key is valid, update status
        const statusElement = document.getElementById('gemini-key-status');
        if (statusElement) {
      ...
      // API request failed - likely an invalid API key
      const statusElement = document.getElementById('gemini-key-status');
      if (statusElement) {
      ...
      if (apiError.response && apiError.response.status === 400) {
        geminiLoading.textContent = "Error: Invalid API key format";
      } else if (apiError.response && apiError.response.status === 403) {
        geminiLoading.textContent = "Error: API key is invalid or not authorized";
      } else {
        geminiLoading.textContent = `Error: ${apiError.message}`;
      }
